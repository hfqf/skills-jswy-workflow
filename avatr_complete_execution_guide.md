...
3. 指数退避重试
 ...
 - 总超时时间30秒，避免无限等待
4.如果遇到需要修改python脚本(比如替换token)执行时，需要在工作空间中创建一个临时目录来执行爬虫任务，而不是直接修改原始文件
...
2. 手机号解密操作:
     - 定位每个手机号旁边的小眼睛图标（ref如e222, e257等）
     - 使用UI点击操作触发手机号解密
     - 成功解密后，手机号从 187****1005 变为完整号码 18795831005
     - 同时出现"复制"按钮和眼睛图标变为可见状态
     - 点完后批量等几秒
     - 再一次性snapshot看结果
     - 不用每点一次就确认一次
...
6.再一次性snapshot看结果
7.不用每点一次就确认一次
8.更新手机号要执行脚本dynamic_update_phones.py
