...
def get_auth_headers(token):
    ...
    return {
        ...
        'login-token': token  # 注意：不是Authorization: Bearer
    }
```

### 5.2 全量线索查询接口
- **URL**: `https://admin.avatr.com/crm/v2/clue/search/pageEsClue`
- **Method**: POST
- **Headers**: 包含 `login-token` 字段
- **Body**: JSON格式，包含分页和时间参数

### 5.3 跟进记录查询接口  
- **URL**: `https://admin.avatr.com/crm/v2/clue/pageClueTouchPlan`
- **Method**: POST
- **Headers**: 同上
- **Body**: 包含 `clueNo` 参数
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            if data.get("code") == 0:
                ...
                print(f"第{page_no}页，获取{actual_count}条数据")
                ...
                if actual_count < page_size:
                    print(f"线索抓取完成！总共处理{total_processed}条数据")
                    break
                else:
                    page_no += 1
            else:
...
def extract_follow_up_records(clue_nos, token, db_path):
    ...
    for i, clue_no in enumerate(clue_nos):
        print(f"处理线索 {i+1}/{len(clue_nos)}: {clue_no}")
...
def save_follow_records_to_db(records, db_path):
    ...
    for record in records:
        cursor.execute("""
            INSERT OR REPLACE INTO follow_records VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            record.get("id"),
            record.get("clueNo"),
            record.get("planTime"),
